...
function initRegionSelects(sidoSelectId, sigunguSelectId) {
    ...
    sidoSelect.addEventListener('change', function() {
        updateSigunguSelect(this.value, sigunguSelectId);
    });
}

function updateSigunguSelect(sidoValue, sigunguSelectId) {
    const sigunguSelect = document.getElementById(sigunguSelectId);
    if (!sigunguSelect) return;

    sigunguSelect.innerHTML = '<option value="">시/군/구 선택</option>';
    if (!sidoValue || !REGION_DATA[sidoValue]) return;

    const sigungus = REGION_DATA[sidoValue];
